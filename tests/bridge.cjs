...
async function scenario(mode) {
 ...
 const kernel={info:Promise.resolve({}),sendInputReply:({value},header)=>{assert.equal(value,'42');assert.equal(header.msg_id,'prompt');f.onIOPub({header:{msg_type:'stream'},content:{name:'stdout',text:'42\n'}});resolveDone({content:{status:'ok'}});},requestExecute:options=>{
  if (options.silent) { assert.equal(options.store_history,false); assert(options.code.includes('ARC_API_KEY')); assert(options.code.includes(Buffer.from('test-secret-only').toString('base64'))); return {done:Promise.resolve({content:{status:'ok'}}),dispose(){}}; }
 ...
 vm.runInNewContext(fs.readFileSync('jupyterlite-bridge/bridge.js','utf8'),{window:{parent,jupyterapp:app,addEventListener:(n,fn)=>listener=fn},location:{origin:'https://fl2744.github.io'},setTimeout,clearTimeout,TextEncoder,btoa:s=>Buffer.from(s,'binary').toString('base64')});
 ...
 const running=send({type:'run-code',code:'print(1)',apiKey:'test-secret-only'});
 ...
 await running; assert.equal(starts,1);assert.equal(shuts,1);assert.equal(saves.length,2);assert(!JSON.stringify(saves).includes('test-secret-only'));assert(!JSON.stringify(saves).includes(Buffer.from('test-secret-only').toString('base64')));
